...
from keras.layers import Input, Dense, Dropout, Lambda, LSTM
...
def load_data(data_path):
    ...
    for i in tqdm(range(5),ncols=80): #num_files
        f = np_files[i]
        if not 'cheek' in f:
            continue
...
def train():
    ...
    x = Lambda(custom_stack_layer, name='lambda_layer')(base_model.output)
    lstm = LSTM(units=256, return_sequences=False, dropout=0.2)(x)
    predict=Dense(1,kernel_initializer='normal')(lstm)
